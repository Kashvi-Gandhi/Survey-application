...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-6 text-zinc-900">
      <div className="flex flex-col sm:flex-row justify-between gap-4 pb-4 border-b border-zinc-200">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900 flex items-center gap-2"><ShieldCheck className="w-6 h-6" /> Admin Dashboard</h1>
          <p className="text-sm text-zinc-600">Monitor QuizPulse activity and manage surveyor access.</p>
        </div>
        <button onClick={loadAdminData} disabled={loading} className="inline-flex self-start items-center gap-1.5 px-3 py-2 text-xs font-semibold text-black bg-white hover:bg-zinc-100 disabled:opacity-50 border border-zinc-200 rounded-md"><RefreshCw className="w-3.5 h-3.5" /> Refresh</button>
      </div>

      <div className="flex flex-wrap gap-1 border-b border-zinc-200">
        {tabs.map((tab) => <button key={tab.id} type="button" onClick={() => setActiveTab(tab.id)} className={`px-4 py-2.5 text-sm font-semibold border-b-2 transition-colors ${activeTab === tab.id ? 'border-black text-black' : 'border-transparent text-zinc-600 hover:text-zinc-900'}`}>{tab.label}</button>)}
      </div>
